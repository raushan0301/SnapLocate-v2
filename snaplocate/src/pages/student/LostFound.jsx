...
export default function LostFoundPage() {
  ...
  const [formData, setFormData] = useState({
    title: '', description: '', location: '', status: 'lost', date: new Date().toISOString().split('T')[0]
  })
  ...
  const handlePost = async (e) => {
    ...
    try {
      ...
      setShowModal(false)
      setFormData({ title: '', description: '', location: '', status: 'lost', date: new Date().toISOString().split('T')[0] })
      fetchItems()
    } catch (err) {
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', width: '100%' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', background:'#fff', padding:'32px 40px', borderRadius:28, boxShadow:'0 1px 4px rgba(0,0,0,.04)', border:'1px solid #f1f5f9', marginBottom: 32 }}>
          ...
          <button 
            ...
            style={{ 
              cursor:'pointer', padding:'12px 24px', borderRadius:16, background:'#ef4444', color:'#fff', border:'none', 
              ...pjs(14, 700, '20px', '#fff'), display:'flex', alignItems:'center', gap: 8, boxShadow:'0 4px 12px rgba(239, 68, 68, 0.2)' 
            }}
          >
            ...
            {filtered.map(item => (
              <div 
                ...
                style={{ 
                  ...
                  boxShadow:'0 1px 3px rgba(0,0,0,.02)', display:'flex', gap: 24, alignItems:'center'
                }}
              >
                <div style={{ width:100, height:100, background:item.status==='lost'?'#fee2e2':'#dcfce7', borderRadius:20, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
        ...
        {showModal && (
          <div style={{ position:'fixed', inset:0, background:'rgba(15,23,42,0.6)', backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, padding: 20 }}>
            <div style={{ background:'#fff', borderRadius: 28, width:'100%', maxWidth: 500, padding: 32, position:'relative' }}>
              <button 
              ...
              <form onSubmit={handlePost} style={{ marginTop: 24, display:'flex', flexDirection:'column', gap: 20 }}>
                <div>
                  ...
                  <input 
                    required type="text" placeholder="e.g. Blue Nike Water Bottle"
                ...
                <div>
                  ...
                  <input 
                    required type="text" placeholder="e.g. Near LT-303 or Library Ground Floor"
                ...
                <div>
                  ...
                  <textarea 
                    rows={3} placeholder="Provide details like color, brand, or unique marks..."
                ...
                <button 
                  disabled={posting}
                  style={{ 
                    marginTop: 8, padding:'14px', borderRadius:14, background:'#0f172a', color:'#fff', border:'none', 
